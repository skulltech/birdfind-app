...
create table lookup_relation_job (
    id bigint generated by default as identity primary key,
    created_at timestamp with time zone default now() not null,
...
create table manage_list_members_job (
    id bigint generated by default as identity primary key,
    created_at timestamp with time zone default now() not null,
    updated_at timestamp with time zone default now() not null,
...
create table manage_relation_job (
    id bigint generated by default as identity primary key,
    created_at timestamp with time zone default now() not null,
    updated_at timestamp with time zone default now() not null,
...
create function get_lookup_relation_jobs_to_add(active_jobs bigint[], failed_jobs bigint[])
...
create function get_manage_relation_jobs_to_add(active_jobs bigint[], failed_jobs bigint[])
...
create function get_manage_list_members_jobs_to_add(active_jobs bigint[], failed_jobs bigint[])
...
create table lookup_tweets_job (
    id bigint generated by default as identity primary key,
    created_at timestamp with time zone default now() not null,
    updated_at timestamp with time zone default now() not null,
    ...
    user_id uuid references auth.users not null,
    target_id bigint references twitter_profile not null,
    since_id bigint references twitter_tweet,
    
    -- To track progress
    paused boolean default false not null,
    finished boolean generated always as (updated_count >= 300) stored,
    deleted boolean default false not null,
    updated_count integer default 0 not null
);

create trigger on_lookup_circle_job_updated
  before update on lookup_circle_job
  for each row execute procedure set_updated_at();

alter table lookup_circle_job enable row level security;

create policy "Users can insert their own manage relation jobs."
  on lookup_circle_job for insert
  to authenticated
  with check ( auth.uid() = user_id );

create policy "Users can view their own manage relation jobs."
  on lookup_circle_job for select
  to authenticated
  using ( auth.uid() = user_id );

create policy "Users can update their own manage relation jobs."
  on lookup_circle_job for update
  to authenticated
  using ( auth.uid() = user_id );
...
create table lookup_circle_job (
    id bigint generated by default as identity primary key,
    created_at timestamp with time zone default now() not null,
    updated_at timestamp with time zone default now() not null,
    ...
    -- To track progress
    paused boolean default false not null,
    member_ids bigint[] default array[]::bigint[]
);
